...
vector<ImplementationManifest> implementations() {
    ...
    result.emplace_back("csr-numa-gcc", "CSR baseline, allocate the internal arrays using all NUMA nodes", &generate_csr_numa);
    result.emplace_back("csr-lcc-numa-gcc", "CSR baseline, allocate the internal arrays using all NUMA nodes, sort-merge impl for the LCC kernel", &generate_csr_lcc_numa);
