...
class Aging {
    ...
    class AgingThread {
        ...
        uint64_t get_num_edges_in_my_partitions() const;

        uint64_t get_num_edges_in_partition(uint64_t part_id) const;

        // Transform the given edge id in the vertex ids src/dst
        void edge_id_2_vertices_id(uint64_t edge_id, uint64_t* out_src_id, uint64_t* out_dst_id);
